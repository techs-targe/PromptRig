...
class AzureGPT4oClient(LLMClient):
    ...
    def __init__(self):
        ...
        # Use GPT-4o specific deployment name
        self.deployment_name = os.getenv("AZURE_OPENAI_GPT4O_DEPLOYMENT_NAME")
        # GPT-4o supports API version 2024-02-15-preview or later
        self.api_version = os.getenv("AZURE_OPENAI_API_VERSION", "2024-02-15-preview")

        # Validate configuration
        if not all([self.endpoint, self.api_key, self.deployment_name]):
            raise ValueError(
                "Azure OpenAI GPT-4o configuration incomplete. "
                "Please set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY, "
                "and AZURE_OPENAI_GPT4O_DEPLOYMENT_NAME in .env file."
            )
