...
# File upload for FILEPATH parameters
...
@router.post("/api/upload/filepath", response_model=Dict[str, str])
async def upload_file_for_filepath(file: UploadFile = File(...)):
    """Upload a file for FILEPATH parameter and return server path.

    Args:
        file: The file to upload

    Returns:
        Dictionary with 'path' containing the server file path
    """
    try:
        ...
        safe_filename = f"{timestamp}_{unique_id}{original_ext}"
        ...
        # Return the relative path (which is what the backend uses)
